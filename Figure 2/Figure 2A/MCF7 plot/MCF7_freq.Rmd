This Script will read the spot segmentation data from the screen for MCF7 cells and generate the relative change in transcription site frequency 
...
total.data_Rab7a <-ldply (paths_Rab7, fread , sep = "\t")
...
Full_Mean<-Two_column_Mean_combined%>%select(c("WellName" ,"Canx" , "Dnajc5" ,    "Egfr"   ,  "Errfi1"  , "Gapdh"  ,  "Kpnb1"  ,  "Myc"   , "Nras"  ,  "P53"  ,    "Rab7a"  ,  "Rpap3"  ,  "RhoA"    , "Sec16a"    ,     "Slc2a1"  )) 

colnames(Full_Mean)


write.csv(Full_Mean, file = 'Output/Collated_Freq_MCF7_raw_unfiltered.csv', row.names = FALSE)
```


```{r}

Full_Mean_b<- read.csv(file = "Output/Collated_Freq_MCF7_raw_unfiltered.csv")
...
MCF7_Freq_Stat_before<-Full_Mean_pivot_b%>%group_by(Genes)%>%dplyr:::summarize(
                                                         median = median(Mean),
...
Segregating replicates and intreated samples 
...
MCF7_Freq_Stat_after<-Full_Mean_pivot%>%group_by(Genes)%>%dplyr:::summarize(
                                                         median = median(Mean),
                                                         high = quantile(Mean, 0.75)+ 5*(quantile(Mean, 0.75)- quantile(Mean, 0.25 )),
                                                         nintynine= quantile(Mean, 0.95),
                                                         five= quantile(Mean, 0.05),
...
MCF7_Freq_Stat_bf <- cbind(MCF7_Freq_Stat_before, MCF7_Freq_Stat_after)
write.csv(MCF7_Freq_Stat_bf, "Output/MCF7_Freq_Stat_beforeAndAfter.csv")
...
Untreated_Data<-Untreated%>%left_join(Full_Mean, by = "WellName")
 ...
 write.csv(Untreated_Data, file = "Output/MCF7_Freq_Untreated.csv", row.names=FALSE)
...
Side_By_side_MCF7<-cbind(Replicate1_Data[c(1,2)], Replicate2_Data[c(2,1)], 
...
replicates_M<-Side_By_side_MCF7[c(1,4,2,3,6,7,10,11,14,15,18,19,22,23,26,27,30,31,34,35,38,39, 42,43,46,47,50,51,54,55)]
...
write.csv(Untreated_Mean_Median_SD, "Output/MCF7_Freq_Untreated_Mean_Median_SD.csv")
...
replicates_average_mean_normalized<-replicates_average_M%>%
  mutate(Canx_F= Canx_F/.354 )%>%
  mutate(Dnajc5_F= Dnajc5_F/.79 )%>%
  mutate(Egfr_F= Egfr_F/.119 )%>%
  mutate(Errfi1_F= Errfi1_F/.350 )%>%
  mutate(Gapdh_F= Gapdh_F/.296 )%>%
  mutate(Kpnb1_F= Kpnb1_F/.952 )%>%
  mutate(Myc_F= Myc_F/.148 )%>%
  mutate(Nras_F= Nras_F/.192 )%>%
  mutate(P53_F= P53_F/.477 )%>%
  mutate(Rab7a_F= Rab7a_F/.427 )%>%
  mutate(Rpap3_F= Rpap3_F/.533 )%>%
  mutate(RhoA_F= RhoA_F/1.068 )%>%
  mutate(Sec16a_F= Sec16a_F/.287 )%>%
mutate(Slc2a1_F= Slc2a1_F/.197)
  


Replicate1_Data_mean_normalized<-Replicate1_Data%>%
   mutate(Canx= Canx/.354 )%>%
  mutate(Dnajc5= Dnajc5/.79 )%>%
  mutate(Egfr= Egfr/.119 )%>%
  mutate(Errfi1= Errfi1/.350 )%>%
  mutate(Gapdh= Gapdh/.296 )%>%
  mutate(Kpnb1= Kpnb1/.952 )%>%
  mutate(Myc= Myc/.148 )%>%
  mutate(Nras= Nras/.192 )%>%
  mutate(P53= P53/.477 )%>%
  mutate(Rab7a= Rab7a/.427 )%>%
  mutate(Rpap3= Rpap3/.533 )%>%
  mutate(RhoA= RhoA/1.068 )%>%
  mutate(Sec16a= Sec16a/.287 )%>%
mutate(Slc2a1= Slc2a1/.197)
...
write.csv(replicates_average_mean_normalized_compounds, file = "Output/MCF7_Freq.csv", row.names = FALSE)

write.csv(Replicate1_Data_mean_normalized, file = "Output//MCF7_Freq_Replicate1.csv", row.names=FALSE)
write.csv(Replicate2_Data_mean_normalized, file = "Output//MCF7_Freq_Replicate2.csv", row.names=FALSE)
...
write.csv(sorted_class, "Output/MCF7_Freq_Class_Sorted_22_98-removed.csv", row.names = FALSE)
...
```{r}
sorted_class <- read.csv("Output/MCF7_Freq_Class_Sorted_22_98-removed.csv", comment.char="#")# row 22 already removed
colnames(sorted_class)
