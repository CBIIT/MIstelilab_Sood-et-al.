...
```{r}
#Filtering the 99 and 1 percentile and then calculating the variance over mean

IntF_Canx <- intensity.data_Canx %>% 
  as_tibble%>% 
...
IntF_Dnajc5 <- intensity.data_Dnajc5 %>% 
  as_tibble%>% 
...
IntF_Errfi1 <- intensity.data_Errfi1 %>% 
  as_tibble%>% 
...
IntF_Gapdh <- intensity.data_Gapdh %>% 
  as_tibble%>% 
...
IntF_Kpnb1 <- intensity.data_Kpnb1 %>% 
  as_tibble%>% 
...
IntF_Rab7a <- intensity.data_Rab7a %>% 
  as_tibble%>% 
...
IntF_Rpap3 <- intensity.data_Rpap3 %>% 
   as_tibble%>% 
   group_by(WellName)%>% filter( Intensity < quantile(Intensity, 0.99) & Intensity > quantile(Intensity, 0.01))%>%dplyr:::summarize ( Mean_Rpap3 = mean (Intensity), SD_Rpap3 = sd(Intensity))%>% dplyr:::mutate(Int_Rpap3 = SD_Rpap3/Mean_Rpap3)


 IntF_RhoA <- intensity.data_RhoA %>% 
   as_tibble%>% 
...
IntF_Sec16a <- intensity.data_Sec16a %>% 
  as_tibble%>% 
...
IntF_Slc2a1 <- intensity.data_Slc2a1 %>% 
  as_tibble%>% 
...
#Combining all the gene Int
 
Mean_Combined <- IntF_Canx%>%
  full_join(IntF_Egfr, by = "WellName")%>%
  full_join(IntF_Errfi1, by = "WellName")%>%
  full_join(IntF_Gapdh, by = "WellName")%>%
  full_join(IntF_Kpnb1, by = "WellName")%>%
  full_join(IntF_Myc, by = "WellName")%>%
  full_join(IntF_P53, by = "WellName")%>%
  full_join(IntF_Rab7a, by = "WellName")%>%
  full_join(IntF_RhoA, by = "WellName")%>%
  full_join(IntF_Sec16a, by = "WellName")%>%
  full_join(IntF_Nras, by = "WellName")%>%
   full_join(IntF_Dnajc5, by = "WellName")%>%
   full_join(IntF_Rpap3, by = "WellName")%>%
  full_join(IntF_Slc2a1, by = "WellName")
 
 #remanimg the columns 

Full_Mean<- Mean_Combined%>%select(WellName, contains("Mean")) %>% dplyr:::rename (
                                                                                   Canx= Mean_Canx,
                                                                      Errfi1=Mean_Errfi1,
                                                                      Egfr=Mean_Egfr,
                                                                      Gapdh=Mean_Gapdh,
...
write.csv(Full_Mean, "Output/Collated_Int_HCT116_raw_unfiltered.csv", row.names = FALSE)
```
...
Full_Mean_b<- read.csv(file = "Output/Collated_Int_HCT116_raw_unfiltered.csv")

sum(is.na(Full_Mean_b))
Full_Mean_pivot_b<-pivot_longer(Full_Mean, c(2:15), names_to = "Genes", values_to = "Mean")%>%na.omit

HCT116_Mean_Stat_before<-Full_Mean_pivot_b%>%group_by(Genes)%>%dplyr:::summarize(    
                                                         median = median(Mean), 
...
# converting the number to NA where the value are 3 times more than the media, use the rannge obatined above 

Full_Mean<- Full_Mean_b%>%
   mutate(Kpnb1 = ifelse(Kpnb1>223.7, NA,Kpnb1 ))%>%
   mutate(P53 = ifelse(P53>135.3, NA,P53 ))
...
HCT116_Mean_Stat_after<-Full_Mean_pivot%>%group_by(Genes)%>%dplyr:::summarize(    
                                                         median = median(Mean), 
                                                         high = quantile(Mean, 0.75)+ 5*(quantile(Mean, 0.75)- quantile(Mean, 0.25 )),max= max(Mean),
                                                         nintynine= quantile(Mean, 0.95),
                                                         five= quantile(Mean, 0.05),
...
Full_Mean<-Full_Mean%>%select(c("WellName" ,"Canx", "Dnajc5" , "Egfr" , "Errfi1",  "Gapdh" , "Kpnb1", "Myc2" ,  "Nras" ,"P53"  ,    "Rab7a" , "Rpap3"  , "RhoA"   ,  "Sec16a"   ,      "Slc2a1"   ))

colnames(Full_Mean)[c(8)]<-"Myc"

HCT116_Mean_Stat_bf <- cbind(HCT116_Mean_Stat_before, HCT116_Mean_Stat_after)

write.csv(HCT116_Mean_Stat_bf, "Output/HCT116_Int_Stat_beforeAndAfter.csv")
...
write.csv(Untreated_Data, "Output/HCT116_Int_Untreated.csv")

#### at this point i check the mean and median



### printing the Interogeneity side by side
...
replicates_Mean<-Side_By_side_HCT116[c(1,4,2,3,6,7,10,11,14,15,18,19,22,23,26,27,30,31,34,35,38,39,42,43, 46,47, 50,51, 54,55)]
colnames(replicates_Mean)

#Taking the mean of replicates

replicates_average_Mean<-replicates_Mean%>%
...
replicates_average_Mean<-replicates_average_Mean%>%select(c(1,2) , contains("_M", ignore.case = TRUE, vars = NULL) )  # change it to include Well name

colnames(replicates_average_Mean)
Mean_Untreated_M<-colMeans(Untreated_Data[sapply(Untreated_Data, is.numeric)], na.rm = TRUE)
...
write.csv(Untreated_Mean_Median_SD, "OUtput/HCT116_Int_Untreated_Mean_Median_SD.csv")
...
```{r}
# normalizing the Mean values
replicates_average_Mean_normalized<-replicates_average_Mean%>%
  mutate(Canx_M= Canx_M/22.39 )%>%
  mutate(Dnajc5_M= Dnajc5_M/95.59 )%>%
  mutate(Egfr_M= Egfr_M/163 )%>%
  mutate(Errfi1_M= Errfi1_M/181.49 )%>%
  mutate(Gapdh_M= Gapdh_M/168.16 )%>%
  mutate(Kpnb1_M= Kpnb1_M/160.27 )%>%
  mutate(Myc_M= Myc_M/100.98)%>%
  mutate(Nras_M= Nras_M/94.58 )%>%
  mutate(P53_M= P53_M/86.08 )%>%
  mutate(Rab7a_M= Rab7a_M/142.3 )%>%
   mutate(Rpap3_M= Rpap3_M/21.22 )%>%
  mutate(RhoA_M= RhoA_M/25.93)%>%
  mutate(Sec16a_M= Sec16a_M/13.98 )%>%
  mutate(Slc2a1_M= Slc2a1_M/25.3 )



Replicate1_Data_Mean_normalized<-Replicate1_Data%>%
  mutate(Canx= Canx/22.39 )%>%
  mutate(Dnajc5= Dnajc5/95.59 )%>%
  mutate(Egfr= Egfr/163 )%>%
  mutate(Errfi1= Errfi1/181.49 )%>%
  mutate(Gapdh= Gapdh/168.16 )%>%
  mutate(Kpnb1= Kpnb1/160.27 )%>%
  mutate(Myc= Myc/100.98)%>%
  mutate(Nras= Nras/94.58 )%>%
  mutate(P53= P53/86.08 )%>%
  mutate(Rab7a= Rab7a/142.3 )%>%
   mutate(Rpap3= Rpap3/21.22 )%>%
  mutate(RhoA= RhoA/25.93)%>%
  mutate(Sec16a= Sec16a/13.98 )%>%
  mutate(Slc2a1= Slc2a1/25.3 )
...
write.csv(replicates_average_Mean_normalized_compounds, "Output/HCT116_Int.csv", row.names= FALSE)




write.csv(Replicate1_Data_Mean_normalized, "Output/HCT116_Int_Replicate1.csv", row.names= FALSE)
write.csv(Replicate2_Data_Mean_normalized, "Output/HCT116_Int_Replicate2.csv", row.names= FALSE)
...
Input: HBEC_Int.csv file from line 659 that has NA un changed 
Output: CSV file with line 22 and 98 removed and compounds class added to the data 

Generating frequency with Class columns

```{r}
data1 <- read.csv("Output/HCT116_Int.csv", comment.char="#")#adding classes and ordering the data frame
...
write.csv(sorted_class, "Output/HCT116_Int_Class_Sorted_22_98-removed.csv", row.names = FALSE)
...
Input: HCT116_Int_Class_Sorted_22_98-removed.csv
...
sorted_class <- read.csv("Output/HCT116_Int_Class_Sorted_22_98-removed.csv", comment.char="#")# row 22 already removed
colnames(sorted_class)
